  ...
  return rawData
    .map((row, index) => {
      ...
      const attendance = parseFloat(
        row.attendance || row.Attendance || row.посещаемость || 0
      )
      const behaviorScore = parseFloat(
        row.behavior || row.behaviorScore || row.поведение || 0
      )
      const participationScore = parseFloat(
        row.participation || row.participationScore || row.участие || 0
      )
      const homeworkCompletion = parseFloat(
        row.homework || row.homeworkCompletion || row.домашнее_задание || 0
      )
      ...
      const student: Student = {
        ...
        participationScore: isNaN(participationScore)
          ? undefined
          : participationScore,
        homeworkCompletion: isNaN(homeworkCompletion)
          ? undefined
          : homeworkCompletion,
      }
  ...
  students.forEach((student, index) => {
    ...
    Object.entries(student.grades).forEach(([subject, grade]) => {
      if (grade !== undefined && (grade < 0 || grade > 100)) {
        warnings.push(
          `Student "${student.name}" has invalid ${subject} grade: ${grade}`
        )
      }
    })
  })
...
export function getDataStats(students: Student[]): {
  ...
  return {
    ...
    averageGradesCount:
      students.length > 0 ? totalGradesCount / students.length : 0,
